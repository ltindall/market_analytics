	...
	try {
		...
		String uberQuery = "SELECT (top.cosSim/(n1.norm*n2.norm)) AS result,prod1.name AS prodA,prod2.name AS prodB FROM "+
				...
				"(SELECT SUM(quantity*price) AS spent, product_id, user_id FROM "+
				 "orders GROUP BY user_id, product_id ORDER BY user_id) AS t1 JOIN "+
				 "(SELECT SUM(quantity*price) AS spent, product_id, user_id FROM "+
				 "orders GROUP BY user_id, product_id ORDER BY user_id) AS t2 "+
				 "ON t1.user_id=t2.user_id AND t1.product_id < t2.product_id GROUP BY t1.product_id,t2.product_id) AS top "+
				 "ON n1.product_id=p1 JOIN "+
				 "(SELECT SUM(quantity*price) AS norm, product_id FROM orders GROUP BY product_id ORDER BY product_id) AS n2 "+
